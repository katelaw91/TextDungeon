	...
	private:
		...
		{ 
    		...
    		{ 
				...
				int x = randomNum(1, rows);
				int y = columns -2;
				MapV[y+1][x] = '_'; //char representing entrance
				MapV[y][x] = ' ';

				//make array of random directions
				int directionArray[8] = {0, 0, 2, 0,3,3,1,1};
				//for(int i = 0; i < 10; i++)
					//directionArray[i] = randomNum(0,3);
				...
				for(int i = 0; i < 8; i++)
					switch(directionArray[i]){
						case 0: //UP
							if(y-2 >= 0) //if y coord is not outside top of map
								{
								if(MapV[y-2][x] == '*'){
									MapV[y-2][x] = ' ';
									MapV[y-1][x] = ' '; }
							...
							break;
						case 1: //DOWN
							if(y+2 <= columns-1) //if y coord is not outside bottom of map
								{
								if(MapV[y+2][x] == '*'){
									MapV[y+2][x] = ' ';
									MapV[y+1][x] = ' '; }
							...
							break;
						case 2: //LEFT
							if(x-2 >= 0) //if x coord is not outside map
								{
								if(MapV[y][x-2] == '*'){
									MapV[y][x-2] = ' ';
									MapV[y][x-1] = ' '; }
							...
							break;
						case 3: //RIGHT
							if(x-2 <= rows -1) //if x coord is not outside map
								{
								if(MapV[y][x+2] == '*') {
									MapV[y][x+2] = ' ';
									MapV[y][x+1] = ' '; }
